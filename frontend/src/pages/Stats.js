        ...
        <div className="max-w-5xl mx-auto p-6 min-h-screen space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <Card className="bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700">
                    ...
                    <CardContent>
                        <div className="text-lg font-bold text-emerald-600 dark:text-emerald-400 truncate">{summary?.best_category}</div>
                    </CardContent>
                </Card>
                <Card className="bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700">
                    <CardHeader className="pb-2">
                            ...
                            activity.map((item, idx) => (
                                <a href={`/exam/${item.id}`} key={item.id} className={`block p-4 hover:bg-slate-50 dark:hover:bg-slate-750 transition-colors ${idx !== activity.length - 1 ? 'border-b border-slate-200 dark:border-slate-700' : ''}`}>
                                    ...
                                    <div className="flex items-center gap-3">
                                        <div className={`p-2 rounded-full ${(item.passed || (item.correct_answers >= Math.ceil((item.total_questions||30)*0.83))) ? 'bg-emerald-100 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400' : 'bg-red-100 dark:bg-red-950/30 text-red-600 dark:text-red-400'}`}>
                                    ...
                                    <div className="text-right">
                                        <div className={`font-bold text-lg ${(item.passed || (item.correct_answers >= Math.ceil((item.total_questions||30)*0.83))) ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}`}>
