...
import { CheckCircle, XCircle, Info, ArrowRight, BookOpen, ArrowLeft, Search, X } from 'lucide-react';
...
export default function Training() {
    ...
    return (
        <div className="min-h-screen pb-20">
            {/* En-tête cohérent avec le reste du site + retour accueil */}
            <header className="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 sticky top-0 z-30 p-4">
                <div className="max-w-3xl mx-auto flex items-center gap-3">
                    ...
                    <h1 className="text-xl font-heading font-bold text-slate-900 dark:text-white">Entraînement</h1>
                </div>
            </header>

            <main className="max-w-3xl mx-auto p-4">
                {/* Contrôles : recherche + catégories, aérés et responsives */}
                <div className="mb-6 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4 shadow-sm">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full rounded-lg border pl-9 pr-9 py-2 text-sm bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-600 text-slate-900 dark:text-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-primary/40"
                            />
                            {search && (
                                <button
                                    type="button"
                        ...
                        >
                            <SelectTrigger aria-label="Ajouter une catégorie">
                    ...
                    <div className="mt-3 flex flex-wrap items-center gap-2">
                        ...
                        {categories.map((c) => (
                            <Badge key={c} variant="secondary" className="cursor-pointer gap-1" onClick={() => {
                                setCategories((prev) => {
