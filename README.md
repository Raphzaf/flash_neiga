...
### Accès au contenu (paywall)

Tout le contenu pédagogique est réservé aux abonnés, **côté serveur** :
questions, panneaux, cours, examens, entraînement, statistiques, erreurs à
retravailler, questions pièges et coach IA exigent un abonnement dont la date
de fin n'est pas dépassée (`require_subscription`). La réponse est un
**402 Payment Required**, que le front traduit par une redirection vers la page
Formules.

Restent accessibles sans abonnement : inscription, connexion, compte, profil et
le tunnel de paiement — sinon un nouvel élève ne pourrait pas s'abonner. Les
comptes listés dans `ADMIN_EMAILS` ne sont pas soumis au paywall.

Un abonnement résilié reste valable jusqu'à sa date de fin : c'est `end_date`
qui fait foi, pas le statut.

### Notifications de paiement HYP

`HYP_REQUIRE_SIGNATURE` vaut **`true` par défaut** : chaque notification de
paiement est renvoyée à HYP (`action=APISign&What=VERIFY`) et n'est acceptée que
si HYP répond `CCode=0`. Sans cette vérification, une notification forgée
suffirait à ouvrir un abonnement sans paiement — l'identifiant de transaction
étant connu de l'acheteur.

Les notifications déjà traitées sont ignorées (idempotence) : un rejeu n'ouvre
pas d'abonnement supplémentaire.

Ne passer `HYP_REQUIRE_SIGNATURE=false` qu'en développement local, où aucune
signature n'est disponible. Jamais en production.
